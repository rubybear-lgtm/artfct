...
up() {
    ...
    # the Worker failed closed and no signed link could open against it. One
    # value per run, handed to the Worker through the same `--var` mechanism
    # as the other secrets and to the Rust integration test through the
    # state env file, so both sides sign and verify with the same key.
    export ARTFCT_ARTIFACT_TOKEN_SECRET="mcp-e2e-artifact-$(openssl rand -hex 16)"
    # The suffix the Worker derives an isolated origin from
    # (`<tenant-slug>--<artifact-id><suffix>`). Production's default is
    # `.artfct.dev`; passing it explicitly keeps the Worker and the
    # integration test on one value even if the caller's shell exports one.
    export ARTFCT_ARTIFACT_ORIGIN_SUFFIX="${ARTFCT_ARTIFACT_ORIGIN_SUFFIX:-.artfct.dev}"
    ...
    # `wrangler dev`'s local proxy replaces the request `Host` with the host it
    # infers from the first `routes` entry — `artfct.dev` here — so an isolated
    # `<slug>--<id>.artfct.dev` request can never reach the Worker, which is
    # the one thing the isolated-origin integration test exists to prove.
    # Nothing configurable turns that inference off (`dev.inferOriginFromRoutes`
    # is not a supported key, and `--routes` with no inferable host is an
    # error), so the dev session runs from the real config with the `routes`
    # block removed. Routes decide which hostnames reach the Worker at the
    # edge; the Worker's own routing is on the path, so this copy serves the
    # same code with the same bindings, vars and build command — it is derived
    # from wrangler.jsonc rather than duplicated, so the two cannot drift.
    # Nothing is parsed here on purpose: JSONC (comments, trailing commas) is
    # wrangler's problem, and a routes block this fails to strip is a loud
    # error rather than a silent Host rewrite.
    node -e '
        const fs = require("node:fs");
        const source = fs.readFileSync("backend/wrangler.jsonc", "utf8");
        const withoutRoutes = source.replace(/"routes"\s*:\s*\[[^\]]*\]\s*,?/s, "");
        if (/"routes"\s*:/.test(withoutRoutes)) throw new Error("the routes block survived the strip");
        fs.writeFileSync("backend/wrangler.e2e.jsonc",
            "// GENERATED by scripts/mcp-e2e-stack.sh: backend/wrangler.jsonc with its\n"
            + "// `routes` block removed, so `wrangler dev` stops replacing the request\n"
            + "// Host with the host of the first route. Do not edit; not for deploy.\n"
            + withoutRoutes);
    '
    (
        cd backend
        ../"$WRANGLER" dev --config wrangler.e2e.jsonc --local --port "$WORKER_PORT" --persist-to "$STATE_DIR/wrangler" \
            --var "ARTFCT_ARTIFACT_TOKEN_SECRET:$ARTFCT_ARTIFACT_TOKEN_SECRET" \
            --var "ARTFCT_ARTIFACT_ORIGIN_SUFFIX:$ARTFCT_ARTIFACT_ORIGIN_SUFFIX" \
...
down() {
    ...
    rm -f backend/wrangler.e2e.jsonc
    rm -rf "$STATE_DIR"
